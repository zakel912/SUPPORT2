...
cleaned_patient_data = patient_data
patient_data
```
...
reg0=lm(formula(regboth),data=cleaned_patient_data)
summary(regboth)
```

```{r}
regbackward=step(res_support2,direction='backward', trace = 0)
reg0=lm(formula(regbackward),data=cleaned_patient_data)
summary(reg0)
```

```{r}
regforward=step(lm(death~1,data=cleaned_patient_data),list(upper=res_support2),direction='forward', trace=0)
reg0=lm(formula(regforward),data=cleaned_patient_data)
summary(reg0)
```


```{r}
colonnes_a_supprimer <- c("slos", "dzclass", "totcst", "totmcst", "meanbp", "wblc", "hrt", "temp", "bili", "sod", "ph", "urine", "adlp", "resp", "diabetes", "race", "scoma","adls", "income", "charges", "avtisst")
cleaned_patient_data <- cleaned_patient_data[, !(names(cleaned_patient_data) %in% colonnes_a_supprimer)]
cor_matrix <- cor(cleaned_patient_data, use = "complete.obs")
cor_matrix[abs(cor_matrix) < 0.4] <- 0
corrplot(cor_matrix, method = "color")
```
```{r}
k_fold_cv <- function(data, K) {
...
# Application avec K = 5 et K = 10
perf_5 <- k_fold_cv(cleaned_patient_data, 5)
perf_10 <- k_fold_cv(cleaned_patient_data, 10)
...
```{r}
set.seed(123)
trainIndex <- createDataPartition(cleaned_patient_data$death, p = 0.8, list = FALSE)
train_data <- cleaned_patient_data[trainIndex, ]
test_data <- cleaned_patient_data[-trainIndex, ]
```

```{r}
library(glmnet)
cv_ridge <- cv.glmnet(as.matrix(patient_data), cleaned_patient_data$death, alpha = 0)
print(cv_ridge$lambda.min)
```
```{r}
# glmnet nécessite une matrice pour les prédicteurs et un vecteur pour la variable cible
