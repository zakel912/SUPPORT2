...
**  dnrday - not useful
  ...
  adlsc - 
```{r}
(sum(is.na(patient_data$ph))/dim(patient_data[0]))*100
```
...
La variable age est importante, cependant on a des floats il faudrait au minimum avoir des entiers, on pourrait aussi catégoriser selon des groupes d'âges, on releve l'age min et max. On a 0 NA  (tranche d'ages : 19-40 ans, 41 à 65 ans, 65 et plus)
```{r}
min(patient_data$age)
max(patient_data$age)
patient_data$age <- round(patient_data$age)
head(patient_data$age)
hist(patient_data$age, breaks=10)
hist(patient_data$age, breaks=3)
sum((patient_data$age>60))

```
On remarque que la majorité des patients ont plus de 60 ans (48%) et 60% ont plus de 65 ans, une portion infime appartient à la tranche d'age <20ans et >100 ans
  

a enlever :
aps sps sur2m sur6m prg2m prg6m dnr dnrday

```{r}
colonnes_a_supprimer <- c("aps", "sps", "sur2m", "sur6m", "prg2m", "prg6m", "dnr", "dnrday")
patient_data_2 <- patient_data[, !(names(patient_data) %in% colonnes_a_supprimer)]
head(patient_data_2)
```
