...
variables_to_normalize <- c('slos', 'd.time', "num.co" ,'scoma', 'charges', 'totcst', 'totmcst', 'avtisst', 'meanbp', 'wblc', 'hrt', 'resp', 'temp', 'pafi', 'alb', 'bili', 'crea', 'sod', 'ph', 'glucose', 'bun', 'adls', 'adlsc', 'urine')
...
Travaille sur la variable target death : 
...
k_fold_cv <- function(data, K) {
  ...
  for (i in 1:K) {
    # Définir les indices du fold courant
    start_index = (i - 1) * foldK + 1
    end_index = if (i == K) n else i * foldK  # Inclure les lignes restantes dans le dernier fold
    indk = start_index:end_index
    ...
    conf_matrix <- table(test_data$death, predicted_class)
    accuracy <- sum(diag(conf_matrix)) / sum(conf_matrix)
    cat("Fold", i, ": Précision =", round(accuracy, 4), "\n")
...
cat("Précision moyenne (5-fold):", round(mean(perf_5), 4), "\n")
cat("Précision moyenne (10-fold):", round(mean(perf_10), 4), "\n")

```

```{r}
# Visualize the results with boxplot
boxplot(perf_5, perf_10, names = c("K = 5", "K = 10"),
        main = "K-Fold Cross-Validation Error Rates",
